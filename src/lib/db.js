...
// Only non-secret columns are selectable client-side (see migration
// 0008_google_oauth.sql) — this is purely "connected or not", never tokens.
export async function getGoogleConnectionStatus(userId) {
  const { data, error } = await supabase
    .from('google_calendar_connections')
    .select('user_id, calendar_id, last_synced_at, connected_by, created_at')
    .eq('user_id', userId)
    .maybeSingle();
  if (error) throw error;
  return data;
