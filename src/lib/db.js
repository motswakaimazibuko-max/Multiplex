...
// Fires an immediate confirmation email to patient + clinician, separate from
// the 24h/2h reminder_queue system. Best-effort: logs but doesn't throw, so a
// notification hiccup never blocks the booking itself from saving.
...
// Only non-secret columns are selectable client-side (see migrations
// 0008_google_oauth.sql / 0009_google_calendar_per_practitioner.sql) — this
// is purely "which practitioners are connected", never tokens. A practice
// can have several connections now — one per practitioner — so this returns
// all of them, keyed by practitioner_id for easy lookup in the UI.
export async function getGoogleConnections(userId) {
  const { data, error } = await supabase
    .from('google_calendar_connections')
    .select('practitioner_id, user_id, google_email, calendar_id, last_synced_at, connected_by, created_at')
    .eq('user_id', userId);
  if (error) throw error;
  const byPractitioner = {};
  for (const row of data || []) byPractitioner[row.practitioner_id] = row;
