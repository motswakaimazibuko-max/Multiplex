...
function PractitionerRow({ p, onEdit, onDelete, googleConfigured, connection, onConnectGoogle, onDisconnectGoogle }) {
  ...
  const handleConnect = async () => {
    ...
    setConnecting(true);
    try {
      await onConnectGoogle(p.id); // redirects the page to Google — this only returns early on error
    } catch (e) {
    ...
    <div style={{ padding: '8px 0', borderBottom: `1px solid ${COLORS.line}` }}>
      ...
      {googleConfigured && (
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginTop: 6, paddingLeft: 22 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 11.5, color: COLORS.slate }}>
            <CalendarCheck2 size={12} color={connection ? COLORS.teal : COLORS.slate} />
            {connection
          ...
          <div style={{ display: 'flex', gap: 6 }}>
            <button
              ...
              disabled={connecting}
              style={{ background: 'none', border: 'none', color: COLORS.teal, cursor: 'pointer', fontSize: 11.5, fontWeight: 600, padding: 4 }}
            >
              {connecting ? 'Connecting…' : connection ? 'Reconnect' : 'Connect'}
            </button>
            {connection && (
              <button
                onClick={handleDisconnect}
                disabled={disconnecting}
                style={{ background: 'none', border: 'none', color: COLORS.rust, cursor: 'pointer', fontSize: 11.5, fontWeight: 600, padding: 4 }}
              >
      ...
      {googleError && <div style={{ color: COLORS.rust, fontSize: 11.5, marginTop: 4, paddingLeft: 22 }}>{googleError}</div>}
    </div>
  );
}
...
export default function SettingsModal({ settings, onClose, onSave, practitioners, onSavePractitioner, onDeletePractitioner, team, myRole, onInviteTeamMember, onRemoveTeamMember, googleConnections, googleConfigured, onConnectGoogle, onDisconnectGoogle }) {
    ...
    <Modal title="Practice details" onClose={onClose}>
      ...
      <div style={{ background: COLORS.paper, border: `1px solid ${COLORS.line}`, borderRadius: 12, padding: 14, marginBottom: 18 }}>
        ...
        <div style={{ fontSize: 12, color: COLORS.slate, marginBottom: 6 }}>
          ...
          {googleConfigured && ' Each practitioner can connect their own Google Calendar below — their bookings sync only to their own calendar.'}
        </div>
        {!googleConfigured && (
          <div style={{ fontSize: 11.5, color: COLORS.slate, marginBottom: 8, fontStyle: 'italic' }}>
            Google Calendar sync isn't set up yet — add Google OAuth credentials (see README) to let practitioners connect their calendars here.
          </div>
        )}
            ...
            {practitioners.map((p) =>
              editingPractitioner?.id === p.id ? (
                ...
                <PractitionerRow
                  key={p.id}
                  p={p}
                  onEdit={setEditingPractitioner}
                  onDelete={onDeletePractitioner}
                  googleConfigured={googleConfigured}
                  connection={googleConnections?.[p.id]}
        ...
        {googleConfigured && (
          <div style={{ fontSize: 11, color: COLORS.slate, marginTop: 10, paddingTop: 10, borderTop: `1px solid ${COLORS.line}` }}>
            Connecting syncs a practitioner's scheduled bookings to their own Google Calendar automatically, and emails the patient an invite. Reschedules or cancellations made directly in Google sync back within about 15 minutes. Prefer not to use Google Calendar? Email reminders (patient 24h before, practitioner 2h before) work independently — see README.
          </div>
        )}
      </div>
