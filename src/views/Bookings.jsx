...
export function ApptFormModal({ appt, clients, practitioners, onClose, onSave, onQuickAddClient }) {
  ...
  const submit = async () => {
    ...
    if (!cId && quickName.trim()) {
      const newClient = await onQuickAddClient({ id: uid(), name: quickName.trim(), phone: '', email: '', notes: '' });
      cId = newClient.id;
    }
    if (!cId) return;
    setSaving(true);
  ...
  return (
    <Modal title={appt ? 'Edit booking' : 'New booking'} onClose={onClose}>
      ...
      {!clientId && (
        <Field label="Or add new client by name">
          <input style={inputStyle} value={quickName} onChange={(e) => setQuickName(e.target.value)} placeholder="Client full name" />
        </Field>
      )}
