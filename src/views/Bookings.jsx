...
export function ApptFormModal({ appt, clients, practitioners, onClose, onSave, onQuickAddClient, initialDate, initialTime }) {
  const patientRecords = clients || []; // defensive: never assume this prop is populated/array
  const [clientId, setClientId] = useState(appt?.client_id || '');
  const [patientMode, setPatientMode] = useState(appt || patientRecords.length ? 'list' : 'new');
  const [patientSearch, setPatientSearch] = useState('');
  const [patientSaving, setPatientSaving] = useState(false);
  const [patientError, setPatientError] = useState('');
  const [wmpSaving, setWmpSaving] = useState(false);
  ...
  const [pIsWmp, setPIsWmp] = useState(false);
  const [pNotes, setPNotes] = useState('');

  const [practitionerId, setPractitionerId] = useState(appt?.practitioner_id || practitioners?.[0]?.id || '');
  const [date, setDate] = useState(appt?.date || initialDate || todayISO());
  const [time, setTime] = useState(appt?.time?.slice(0, 5) || initialTime || '09:00');
  ...
  const selectedClient = patientRecords.find((c) => c.id === clientId);
  const filteredClients = patientSearch.trim()
    ? patientRecords.filter((c) => (c.name || '').toLowerCase().includes(patientSearch.trim().toLowerCase()))
    : patientRecords;

  // Loaded patient's WMP status is persisted straight to their client record
  // (via the same upsert used for new patients / quick-add), so it sticks
  // across every future booking, not just this one.
  const toggleSelectedWmp = async () => {
    if (!selectedClient) return;
    setWmpSaving(true);
    setPatientError('');
    try {
      await onQuickAddClient({ ...selectedClient, is_wmp: !selectedClient.is_wmp });
    } catch (e) {
      setPatientError(e.message || 'Could not update WMP status');
    } finally {
      setWmpSaving(false);
    }
  };
    ...
    <Modal title={appt ? 'Edit booking' : 'New booking'} onClose={onClose}>
      ...
      {patientRecords.length > 0 && (
        <div style={{ display: 'flex', justifyContent: 'flex-end', marginBottom: 10 }}>
          <button
            type="button"
            onClick={() => setPatientMode(patientMode === 'new' ? 'list' : 'new')}
            style={{ background: 'none', border: 'none', color: COLORS.teal, fontSize: 12.5, fontWeight: 600, cursor: 'pointer', display: 'inline-flex', alignItems: 'center', gap: 4, padding: 0 }}
          >
            {patientMode === 'new' ? (<><Users size={13} /> Choose existing patient</>) : (<><UserPlus size={13} /> Add new patient</>)}
          </button>
        </div>
      )}
      ...
      {patientMode === 'list' ? (
            ...
            <>
              ...
              <Field label="Patient">
                <select style={inputStyle} value={clientId} onChange={(e) => setClientId(e.target.value)}>
                  ...
                  {filteredClients.map((c) => <option key={c.id} value={c.id}>{c.name}{c.is_wmp ? ' · WMP' : ''}</option>)}
                </select>
              </Field>
              {filteredClients.length === 0 && (
                <div style={{ fontSize: 13, color: COLORS.slate, marginTop: -8, marginBottom: 14 }}>No patients match "{patientSearch}".</div>
              )}
              {selectedClient?.medical_aid_scheme && (
                <div style={{ fontSize: 12, color: COLORS.slate, marginTop: -8, marginBottom: 10 }}>
                  Medical aid: {selectedClient.medical_aid_scheme}{selectedClient.medical_aid_number ? ` · ${selectedClient.medical_aid_number}` : ''}
                </div>
              )}
              {selectedClient && (
                <label style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: 13, color: COLORS.ink, marginBottom: 14, cursor: wmpSaving ? 'default' : 'pointer' }}>
                  <input type="checkbox" checked={!!selectedClient.is_wmp} onChange={toggleSelectedWmp} disabled={wmpSaving} style={{ width: 16, height: 16 }} />
                  Weight Management Program (WMP){wmpSaving ? ' — saving…' : ''}
                </label>
              )}
            </>
          )}
        ...
        <>
          ...
          <label style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: 13, color: COLORS.ink, margin: '4px 0 14px', cursor: 'pointer' }}>
            <input type="checkbox" checked={pIsWmp} onChange={(e) => setPIsWmp(e.target.checked)} style={{ width: 16, height: 16 }} />
      ...
      <div style={{ display: 'flex', gap: 10 }}>
        ...
        <div style={{ flex: 1 }}><Field label="Time"><input type="time" step={1800} min="05:00" max="20:00" style={inputStyle} value={time} onChange={(e) => setTime(e.target.value)} /></Field></div>
      </div>
      <div style={{ display: 'flex', gap: 10 }}>
        <div style={{ flex: 1 }}>
