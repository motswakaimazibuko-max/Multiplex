...
export default function CalendarView({ appointments, clients, practitioners, onEdit, onNew, onReschedule }) {
  ...
  return (
    <div>
      ...
      <div style={{ display: 'flex', gap: 24, alignItems: 'flex-start', flexWrap: 'wrap' }}>
        ...
        <div style={{ flex: '0 0 300px', width: '100%', maxWidth: 320 }}>
          ...
          <div style={{ display: 'flex', gap: 14, marginBottom: 10, fontSize: 12, color: COLORS.slate, flexWrap: 'wrap' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(7, 1fr)', gap: 4, marginBottom: 18, border: `1px solid ${COLORS.line}`, borderRadius: 14, background: COLORS.paperRaised, padding: 8 }}>
            {cells.map((day, i) => {
              ...
              return (
                <button
                  ...
                  style={{
                    ...
                    background: isSelected ? '#E6EFEC' : isToday ? '#EEF3F0' : COLORS.paperRaised,
                    cursor: 'pointer',
                    minHeight: 52,
                  }}
                >
        ...
        {/* Center panel: selected day agenda with times and clinician names */}
        <div style={{ flex: '1 1 380px', minWidth: 320 }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 8 }}>
            <div style={{ fontFamily: 'Newsreader, serif', fontSize: 19, color: COLORS.ink }}>{selectedLabel}</div>
          ...
          if (!occ) {
            ...
            // or drop a dragged appointment here to reschedule it to this time.
            const isDropTarget = draggingId && dragOverSlot === slotStart;
            return (
              <button
                key={slotStart}
                ...
                onDragLeave={() => {
                  setDragOverSlot((s) => (s === slotStart ? null : s));
                }}
                onDrop={(e) => {
                  e.preventDefault();
                  const id = e.dataTransfer.getData('text/plain');
                  const dragged = selectedAppts.find((a) => a.id === id);
                  setDraggingId(null);
                  setDragOverSlot(null);
                  if (dragged) onReschedule?.(dragged, label);
                }}
                style={{
                  width: '100%',
                  display: 'flex',
                  ...
                  gap: 10,
                  padding: '9px 14px',
                  background: isDropTarget ? '#E6EFEC' : 'none',
                  border: 'none',
                  borderTop,
                  cursor: 'pointer',
                  textAlign: 'left',
              ...
              >
                ...
                <span style={{ fontSize: 12, color: isDropTarget ? COLORS.teal : COLORS.line, flex: 1, borderTop: `1px dashed ${isDropTarget ? COLORS.teal : COLORS.line}` }} />
          ...
          return (
            <div
              ...
              style={{
                borderTop,
                borderLeft: `4px solid ${wmp ? COLORS.wmpGreen : COLORS.calendarBlue}`,
