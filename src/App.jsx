...
import { Calendar, CalendarDays, FileText, Settings as SettingsIcon, Users, LogOut } from 'lucide-react';
import { supabase } from './supabaseClient';
import Auth from './Auth';
import { COLORS } from './theme';
import * as db from './lib/db';
import { disconnectGoogleCalendar, pushAppointmentToGoogle, startGoogleOAuth } from './lib/googleCalendar';
...
function PracticeApp({ userId, authUserId, role }) {
  ...
  const [googleConnections, setGoogleConnections] = useState({}); // { [practitionerId]: {last_synced_at, google_email, ...} }
  const [googleBanner, setGoogleBanner] = useState(null); // {type:'connected'|'error', message}

  // Server-side OAuth — connecting just redirects to Google; the actual
  // token exchange happens in the google-oauth-callback function, and the
  // browser comes back to `?google=connected` (handled in the effect below).
  // Each practitioner connects their own account, so both calls need to know
  // which practitioner they're acting on.
  const connectGoogleCalendar = async (practitionerId) => {
    await startGoogleOAuth(practitionerId);
  };
  const disconnectGoogle = async (practitionerId) => {
    await disconnectGoogleCalendar(practitionerId);
  ...
  // Pick up the redirect back from google-oauth-callback (?google=connected
  // or ?google=error&message=...), show a banner, then clean the URL.
  useEffect(() => {
    const params = new URLSearchParams(window.location.search);
    const result = params.get('google');
    if (!result) return;
    if (result === 'connected') {
      setGoogleBanner({ type: 'connected', message: 'Google Calendar connected — that practitioner\u2019s bookings will sync automatically.' });
      db.getGoogleConnections(userId).then(setGoogleConnections).catch(() => {});
    } else if (result === 'error') {
      setGoogleBanner({ type: 'error', message: params.get('message') || 'Could not connect Google Calendar.' });
    }
  ...
  const refreshAll = useCallback(async () => {
    const [c, a, i, s, p, t, g] = await Promise.all([
      ...
      db.listTeam(userId),
      db.getGoogleConnections(userId).catch(() => ({})),
    ]);
    setClients(c);
    setAppointments(a);
    setInvoices(i);
  ...
  // ---- appointments ----
  // Google sync now happens server-side (google-calendar-push), keyed off
  // the saved appointment's id — the edge function itself decides push vs.
  // delete based on status, and is a no-op if Google isn't connected.
  // Only push if the appointment's own practitioner has a Google connection
  // — there's no practice-wide fallback calendar anymore, so an appointment
  // with no practitioner (or an unconnected one) just doesn't sync, silently.
  const saveAppt = async (appt) => {
    const saved = await db.upsertAppointment(userId, appt);
    if (saved.practitioner_id && googleConnections[saved.practitioner_id]) pushAppointmentToGoogle(saved.id, 'upsert');
    await refreshAll();
  };
  const removeAppt = async (id) => {
    // Push the delete before removing the local row — the edge function
    // needs the appointment (and its google_event_id) to still exist to know
    // what to cancel in Google. The push edge function itself checks the
    // appointment's practitioner and no-ops if unconnected, so it's safe to
    // always attempt this rather than re-deriving practitioner_id here.
    await pushAppointmentToGoogle(id, 'delete');
  ...
  return (
    <div style={{ minHeight: '100vh', background: COLORS.paper, fontFamily: 'Inter, sans-serif', paddingBottom: 90 }}>
      ...
      <main style={{ padding: '18px 16px 40px', maxWidth: tab === 'calendar' ? 1080 : 720, margin: '0 auto' }}>
        {googleBanner && (
          <div
            style={{
              background: googleBanner.type === 'connected' ? '#EEF3F0' : '#FBEAE6',
              border: `1px solid ${googleBanner.type === 'connected' ? COLORS.teal : COLORS.rust}`,
              color: googleBanner.type === 'connected' ? COLORS.tealDeep : COLORS.rust,
              borderRadius: 10,
              padding: '10px 12px',
              fontSize: 13,
              marginBottom: 14,
          ...
          >
            ...
            <button onClick={() => setGoogleBanner(null)} style={{ background: 'none', border: 'none', color: 'inherit', cursor: 'pointer', fontWeight: 700 }}>×</button>
          </div>
        )}
        {tab === 'bookings' && (
          <BookingsView
            ...
            onNew={() => { setEditingAppt(null); setNewApptSlot(null); setShowApptForm(true); }}
            onEdit={(a) => { setEditingAppt(a); setNewApptSlot(null); setShowApptForm(true); }}
            onDelete={removeAppt}
        ...
        {tab === 'calendar' && (
          <CalendarView
            ...
            practitioners={practitioners}
            onEdit={(a) => { setEditingAppt(a); setNewApptSlot(null); setShowApptForm(true); }}
            onNew={(date, time) => { setEditingAppt(null); setNewApptSlot(date ? { date, time } : null); setShowApptForm(true); }}
            onReschedule={(appt, time) => saveAppt({ ...appt, time })}
          />
        )}
        {tab === 'clients' && (
          <ClientsView
      ...
      {showApptForm && (
        <ApptFormModal
          ...
          onClose={() => { setShowApptForm(false); setNewApptSlot(null); }}
          onSave={async (a) => { await saveAppt(a); setShowApptForm(false); setNewApptSlot(null); }}
