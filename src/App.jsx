...
function PracticeApp({ userId, authUserId, role }) {
  ...
  const [googleStatus, setGoogleStatus] = useState(null); // null = not connected, else {last_synced_at, ...}
  ...
  useEffect(() => {
    ...
    if (!result) return;
    if (result === 'connected') {
      setGoogleBanner({ type: 'connected', message: 'Google Calendar connected — bookings will sync automatically.' });
      db.getGoogleConnectionStatus(userId).then(setGoogleStatus).catch(() => {});
  ...
  const refreshAll = useCallback(async () => {
    const [c, a, i, s, p, t, g] = await Promise.all([
      ...
      db.listTeam(userId),
      db.getGoogleConnectionStatus(userId).catch(() => null),
    ]);
    setClients(c);
    setAppointments(a);
    setInvoices(i);
  ...
  const saveAppt = async (appt) => {
    ...
    if (googleStatus) pushAppointmentToGoogle(saved.id, 'upsert');
    await refreshAll();
  };
  const removeAppt = async (id) => {
    ...
    // what to cancel in Google.
    if (googleStatus) await pushAppointmentToGoogle(id, 'delete');
    ...
    <div style={{ minHeight: '100vh', background: COLORS.paper, fontFamily: 'Inter, sans-serif', paddingBottom: 90 }}>
      ...
      {showSettings && (
        <SettingsModal
          ...
          googleLastSyncedAt={googleStatus?.last_synced_at}
