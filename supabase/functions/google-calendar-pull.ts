...
// bottom of migration 0008_google_oauth.sql). For every connected
// practitioner (each practice can have several — see
// 0009_google_calendar_per_practitioner.sql), asks Google what changed since
// the last check and reflects reschedules or cancellations back onto the
// matching local appointment.
//
...
async function syncOneConnection(admin: any, connection: any) {
  ...
  // Keyed by practitioner_id (the real primary key) — user_id is shared by
  // every practitioner in the same practice, so using it here would touch
  // every connection in the practice instead of just this one.
  await admin.from('google_calendar_connections').update({
...
Deno.serve(async () => {
  try {
    ...
    for (const connection of connections || []) {
      try {
        results[connection.practitioner_id] = await syncOneConnection(admin, connection);
      } catch (e) {
        results[connection.practitioner_id] = { error: String(e.message || e) };
      }
    }
