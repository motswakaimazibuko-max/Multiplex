...
// verify state, exchange the code for tokens, store them against the
// practitioner named in `state`, then redirect the browser back into the app.
//
...
async function verifyState(state: string): Promise<{ practitionerId: string; uid: string; by: string; ts: number }> {
  ...
  if (!payload.practitionerId) throw new Error('State missing practitionerId');
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { practitionerId: pid, uid: practiceOwnerId, by: connectedBy } = await verifyState(state);
    practitionerId = pid;
    ...
    // Fetch which Google account this actually is, purely so Settings can
    // show "Connected — jane@gmail.com" instead of just "Connected" — useful
    // now that a practice can have several different accounts linked.
    ...
    return redirectTo(`${fallbackApp}?google=connected&practitioner=${practitionerId}`);
  } catch (e) {
    const params = new URLSearchParams({ google: 'error', message: String(e.message || e) });
    if (practitionerId) params.set('practitioner', practitionerId);
    return redirectTo(`${fallbackApp}?${params.toString()}`);
  }
});
