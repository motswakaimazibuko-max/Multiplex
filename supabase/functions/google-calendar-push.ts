...
//
// Each practitioner has their own Google connection (see
// 0009_google_calendar_per_practitioner.sql), so a booking only syncs if the
// specific practitioner assigned to it has connected their calendar —
// there's no more single practice-wide calendar to fall back to.
//
...
async function refreshAccessToken(admin: any, connection: any) {
  ...
  }).eq('practitioner_id', connection.practitioner_id);
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!appt.practitioner_id) return ok({ ok: false, reason: 'no_practitioner' }); // nobody to sync to — no calendar it could belong to
    ...
    const { data: connection } = await admin
      ...
      .maybeSingle();
    if (!connection) return ok({ ok: false, reason: 'practitioner_not_connected' });
    ...
    // The event lives on the practitioner's own calendar now (each
    // practitioner connects their own account), so only the patient needs
    // an attendee invite — adding the practitioner too would just be a
    // redundant self-invite email to the account that already owns the event.
    const attendees = [];
